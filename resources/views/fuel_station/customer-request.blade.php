    ...
    <h1>Fuel Tokens</h1>
      <nav>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Fuel Tokens</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
                  ...
                  <table class="table table-striped table-hover table-responsive">
                    <thead>
                      <tr>
                        ...
                        <!-- <th>Approve</th>
                        <th>Decline</th> -->
                        <th style="text-align: center; justify-content: center;">Action</th>
                      </tr>
                    </thead>
                    <tbody>
                      @foreach ($fuelRequests as $request)
                        <tr>
                          <th>{{ $request->Fuel_Request_ID }}</th>
                          ...
                          <td style="justify-items: center;">
                            @if ($request->status == 'Pending')
                              <div class="btn-group">
                                <form action="{{ route('fuel-requests.approve', $request->Fuel_Request_ID) }}" method="POST">
                                  @csrf
                            ...
                            @elseif ($request->status == 'Approved' && !empty($request->Scheduled_Filling_Date) && !empty($request->Scheduled_Filling_Time))
                              <form action="{{ route('tokens.view', $request->Fuel_Request_ID) }}" method="POST">
                                ...
                                <button type="submit" class="btn btn-success btn-sm">View Token</button>
                              </form>
                            @elseif ($request->status == 'Approved')
                              <form action="{{ route('tokens.create') }}" method="POST">
                                @csrf
                                <input id="fuel_request_id" name="fuel_request_id" type="hidden" value="{{ $request->Fuel_Request_ID }}"/>
                                <button type="submit" class="btn btn-info btn-sm">Create Token</button>
                              </form>
                            @endif
                          </td>
                        </tr>
